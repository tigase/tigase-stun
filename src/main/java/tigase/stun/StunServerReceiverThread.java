...
public class StunServerReceiverThread
	...
	public StunServerReceiverThread(StunSocket receiver, Vector<StunSocket> sockets,
		...
		for (StunSocket socket : sockets) {
			if ((socket.getLocalPort() != receiverSocket.getLocalPort()) &&
					(socket.getLocalAddress() == receiverSocket.getLocalAddress())) {
				changedPort = socket;
			}
			if ((socket.getLocalPort() != receiverSocket.getLocalPort()) &&
					(socket.getLocalAddress() != receiverSocket.getLocalAddress())) {
			...
			try {
				...
				try {
					...
					if (header.getType() == MessageHeaderType.BindingRequest) {
						ChangeRequest cr = (ChangeRequest) header.getMessageAttribute(
								MessageAttributeType.ChangeRequest);
						if (cr == null) {
							throw new MessageAttributeException("Message attribute change request is not set.");
						}
						ResponseAddress ra = (ResponseAddress) header.getMessageAttribute(
